...
def load_config() -> dict:
    ...
    gsheet_tab = os.getenv("GSHEET_TAB", "TELE_PSI")
    creds_json = os.getenv("CREDS_JSON")

    download_dir = os.getenv("DOWNLOAD_DIR", "").strip()
    download_dir_base = os.getenv(
        "DOWNLOAD_DIR_BASE",
        "/mnt/BD_CControl/BASE TCEX 2026/OBSTETRICIA",
    ).strip()
    ...
    servicio = os.getenv("SERVICIO", "F21").strip()
    ...
    # motor
    run_psicoprofilaxis = os.getenv("RUN_PSICOPROFILAXIS", "true").lower() == "true"
    ...
    # Paso 4: ejecución
    run_name = os.getenv("RUN_NAME", "TELE_PSICOPROFILAXIS")
    ...
    for k, v in [
        ("URL_HOME", url_home),
        ("FRM_PSICOPROFILAXIS", frm_psicoprofilaxis),
    ...
    return {
        ...
        "RUN_PSICOPROFILAXIS": run_psicoprofilaxis,
...
def build_routes_for_month(config: dict, month_ctx: dict) -> List[Dict[str, str]]:
    routes = []
    if config.get("RUN_PSICOPROFILAXIS"):
        final_dir = resolve_download_dir(config, month_ctx)
        ensure_dir(final_dir)
        routes.append({
            "code": "PSICOPROFILAXIS",
            "name": "Tele Psicoprofilaxis",
            "url": config["FRM_PSICOPROFILAXIS"],
...
def main():
    ...
    if not rutas:
        logger.info("RUN_SKIP | RUN_PSICOPROFILAXIS=false. No hay nada que ejecutar.")
        sys.exit(1)
    ...
    if not centros_meta:
        ...
        send_run_email(
            logger,
            config,
            "[OK] RPA Tele Psicoprofilaxis | exit_code=0",
    ...
    send_run_email(
        logger,
        config,
        f"[{subject_status}] RPA Tele Psicoprofilaxis | exit_code={exit_code}",
