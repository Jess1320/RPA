...
def publish_month_txts_to_final_dir(
    ...
    try:
        ...
        # Cronicos comparte carpeta final con el Mensual de madrugada. Por eso
        # no debe limpiar el periodo completo ni otros centros ya publicados:
        # solo actualiza los archivos descargados en esta corrida.
        removed = 0
...
def ejecutar_descargas_por_macro(
    ...
    for macro in VALID_MACROS:
        ...
        # Mantener el mismo comportamiento operativo del RPA Diario:
        # el healthcheck no bloquea macros porque puede dar falsos negativos
        # aunque la descarga real funcione correctamente.
        health = {"status": "SKIPPED", "detail": "Health check desactivado siguiendo flujo del RPA Diario"}
...
def load_config_from_env() -> dict:
    ...
    DOWNLOAD_ONLY = _env_bool("DOWNLOAD_ONLY", "false")
    ...
    HEALTHCHECK_BLOCKING = _env_bool("HEALTHCHECK_BLOCKING", "false")
    ...
    MAX_CENTERS = _env_int("MAX_CENTERS", "0")
    CENTERS_FILTER_RAW = _env_str("CENTERS_FILTER", "")
...
def main():
    ...
    summary(summary_path, "MODE | MONTHLY_CRONICOS" if cfg["DOWNLOAD_ONLY"] else "MODE | MONTHLY")
    ...
    for ctx in MESES_CTX:
        ...
        if DB_PRECHECK_ENABLED and not DOWNLOAD_ONLY:
        ...
        if DOWNLOAD_ONLY:
            summary(summary_path, "DB_SKIP | DOWNLOAD_ONLY=true")
        else:
            try:
                db = PgRPAControl(
                    host=cfg["PG_HOST"],
        ...
        if cfg["CENTERS_FILTER"]:
            allowed = set(cfg["CENTERS_FILTER"])
            before_filter = len(centros_meta)
            centros_meta = [x for x in centros_meta if x["centro"] in allowed]
            summary(
                summary_path,
                f"CENTERS_FILTER | before={before_filter} | after={len(centros_meta)} | filter={cfg['CENTERS_FILTER']}"
            )

        if cfg["MAX_CENTERS"] and cfg["MAX_CENTERS"] > 0:
            before_limit = len(centros_meta)
            centros_meta = centros_meta[: cfg["MAX_CENTERS"]]
            summary(
                summary_path,
                f"MAX_CENTERS | before={before_limit} | after={len(centros_meta)} | limit={cfg['MAX_CENTERS']}"
            )
