...
def cleanup_empty_dirs(root_dir: str, logger=None) -> int:
    root = Path(root_dir)
    if not root.is_dir():
        return 0
    removed = 0
...
def load_config() -> dict:
    ...
    frm_hospitalizacion = os.getenv(
        "FRM_HOSPITALIZACION",
        "http://appsgasistexpl.essalud.gob.pe/explotaDatos/servlet/CtrlControl?opt=hosinterser",
    ).strip()
    frm_emergencia = os.getenv(
        "FRM_EMERGENCIA",
        "http://appsgasistexpl.essalud.gob.pe/explotaDatos/servlet/CtrlControl?opt=emerinterconsulta",
    ).strip()
    ...
    gsheet_tab = os.getenv("GSHEET_TAB", "TELE_PROAD")
    creds_json = os.getenv("CREDS_JSON")

    download_dir_hosp = os.getenv("DOWNLOAD_DIR_HOSPITALIZACION")
    download_dir_emer = os.getenv("DOWNLOAD_DIR_EMERGENCIA")
    ...
    servicio = os.getenv("SERVICIO", "AJ1").strip()
    formato_archivo = os.getenv("FORMATO_ARCHIVO", "xls").strip()
    suffix_hosp = os.getenv("FILE_SUFFIX_HOSPITALIZACION", "IntercxServ").strip()
    suffix_emer = os.getenv("FILE_SUFFIX_EMERGENCIA", "EmerInterconsultaxServ").strip()

    # motor
    run_hosp = os.getenv("RUN_HOSPITALIZACION", "true").lower() == "true"
    run_emer = os.getenv("RUN_EMERGENCIA", "true").lower() == "true"
    ...
    # Paso 4: ejecución
    run_name = os.getenv("RUN_NAME", "TELE_PROAD")
    ...
    for k, v in [
        ("URL_HOME", url_home),
        ("FRM_HOSPITALIZACION", frm_hospitalizacion),
        ("FRM_EMERGENCIA", frm_emergencia),
        ("GSHEET_URL", gsheet_url),
        ("GSHEET_TAB", gsheet_tab),
        ("CREDS_JSON", creds_json),
        ("DOWNLOAD_DIR_HOSPITALIZACION", download_dir_hosp),
        ("DOWNLOAD_DIR_EMERGENCIA", download_dir_emer),
    ...
    if not suffix_hosp:
        faltan.append("FILE_SUFFIX_HOSPITALIZACION")
    if not suffix_emer:
        faltan.append("FILE_SUFFIX_EMERGENCIA")
    ...
    ensure_dir(logs_dir)
    ensure_dir(download_dir_hosp)
    ensure_dir(download_dir_emer)
    ...
    if run_hosp:
        routes.append({
            ...
            "url": frm_hospitalizacion,
            "download_dir": download_dir_hosp,
            "expected_suffix": suffix_hosp,
        })
    if run_emer:
        routes.append({
            "code": "EMERGENCIA",
            "name": "Emergencia",
            "url": frm_emergencia,
            "download_dir": download_dir_emer,
            "expected_suffix": suffix_emer,
        })
    ...
    return {
        ...
        "FILE_SUFFIX_HOSPITALIZACION": suffix_hosp,
        "FILE_SUFFIX_EMERGENCIA": suffix_emer,

        "RUN_HOSPITALIZACION": run_hosp,
        "RUN_EMERGENCIA": run_emer,
...
def main():
    ...
    if not rutas:
        logger.info("RUN_SKIP | RUN_HOSPITALIZACION y RUN_EMERGENCIA estan en false. No hay nada que ejecutar.")
        sys.exit(1)
    ...
    try:
        ...
        purge_old_partials(logger, config["DOWNLOAD_DIR_HOSPITALIZACION"], max_age_minutes=config["PURGE_PARTIALS_AGE_MIN"])
        purge_old_partials(logger, config["DOWNLOAD_DIR_EMERGENCIA"], max_age_minutes=config["PURGE_PARTIALS_AGE_MIN"])
        cleanup_empty_dirs(config["STAGING_DOWNLOAD_DIR"], logger)
        kill_chromedrivers()
        removed_profiles = cleanup_old_chrome_profiles(
            config["CHROME_PROFILE_BASE_DIR"],
    ...
    send_run_email(
        logger,
        config,
        f"[{subject_status}] RPA Tele Proad | exit_code={exit_code}",
