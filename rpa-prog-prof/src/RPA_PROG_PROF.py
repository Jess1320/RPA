...
def load_config() -> dict:
    ...
    gsheet_tab = os.getenv("GSHEET_TAB", "CENACRON")
    creds_json = os.getenv("CREDS_JSON")

    download_dir_prog_prof = os.getenv(
        "DOWNLOAD_DIR_PROG_PROF",
    ...
    suffix_prog_prof = os.getenv("FILE_SUFFIX_PROG_PROF", "ConsProgPro").strip()
    ...
    # Paso 4: ejecución
    run_name = os.getenv("RUN_NAME", "PROG_PROF")
    ...
    for k, v in [
        ("URL_HOME", url_home),
        ("FRM_PROG_PROF", frm_prog_prof),
        ("GSHEET_URL", gsheet_url),
        ("GSHEET_TAB", gsheet_tab),
        ("CREDS_JSON", creds_json),
        ("DOWNLOAD_DIR_PROG_PROF", download_dir_prog_prof),
    ...
    if not suffix_prog_prof:
        faltan.append("FILE_SUFFIX_PROG_PROF")
    ...
    ensure_dir(logs_dir)
    ensure_dir(download_dir_prog_prof)
    ensure_dir(staging_download_dir)

    routes = [{
        "code": "PROG_PROF",
        "name": "Programacion de Profesionales",
        "url": frm_prog_prof,
        "download_dir": download_dir_prog_prof,
        "expected_suffix": suffix_prog_prof,
    ...
    return {
        ...
        "FILE_SUFFIX_PROG_PROF": suffix_prog_prof,
...
def main():
    ...
    if not rutas:
        logger.info("RUN_SKIP | No hay rutas Prog Prof habilitadas. No hay nada que ejecutar.")
        sys.exit(1)
    ...
    send_run_email(
        logger,
        config,
        f"[{subject_status}] RPA Prog Prof | exit_code={exit_code}",
