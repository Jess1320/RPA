...
def sanitize_staging_value(raw_value) -> str:
    value = "" if raw_value is None else str(raw_value)
    value = re.sub(r"[\r\n\t]+", " ", value)
    return value.strip()


def compact_row_sample(row: list, max_cols: int = 10, max_chars: int = 240) -> str:
    sample = "|".join(sanitize_staging_value(v) for v in row[:max_cols])
    if len(row) > max_cols:
        sample += "|..."
    ...
    with open(file_path, "r", encoding="utf-8-sig", errors="replace", newline="") as f:
        ...
        for row in reader:
            ...
            for idx, raw_value in enumerate(row):
                ...
                value = sanitize_staging_value(raw_value)
