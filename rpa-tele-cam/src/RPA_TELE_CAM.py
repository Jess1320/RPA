...
def publish_validated_download(
    ...
    deleted = clean_existing_finals(logger, final_dir, prefix, expected_suffix)
    if deleted:
...
def load_config() -> dict:
    ...
    frm_telecam = os.getenv(
        "FRM_TELECAM",
        "http://appsgasistexpl.essalud.gob.pe/explotaDatos/servlet/CtrlControl?opt=cext13",
    ).strip()
    ...
    input_source = _norm_cell(os.getenv("INPUT_SOURCE", "DIRECT"))
    gsheet_url = os.getenv("GSHEET_URL")
    gsheet_tab = os.getenv("GSHEET_TAB", "TELE_CAM")
    ...
    tipo_atencion = os.getenv("TIPO_ATENCION", "2").strip()
    mostrar_por = os.getenv("MOSTRAR_POR", "2").strip()
    servicio = os.getenv("SERVICIO", "F41").strip()
    actividad = os.getenv("ACTIVIDAD", "A1").strip()
    subactividad = os.getenv("SUBACTIVIDAD", "142").strip()
    formato_archivo = os.getenv("FORMATO_ARCHIVO", "xls").strip()
    file_suffix = os.getenv("FILE_SUFFIX", "AtenNoMedxSubAct").strip()

    # motor
    run_telecam = os.getenv("RUN_TELECAM", "true").lower() == "true"
    ...
    # Paso 4: ejecución
    run_name = os.getenv("RUN_NAME", "TELE_CAM")
    max_centers = int(os.getenv("MAX_CENTERS", "0"))
    centers_filter_raw = os.getenv("CENTERS_FILTER", "").strip()
    direct_centers_raw = os.getenv("DIRECT_CENTERS", centers_filter_raw or "739").strip()
    centers_filter = [c.strip() for c in centers_filter_raw.split(",") if c.strip()] if centers_filter_raw else []
    direct_centers = [c.strip() for c in direct_centers_raw.split(",") if c.strip()] if direct_centers_raw else []
    ...
    for k, v in [
        ("URL_HOME", url_home),
        ("FRM_TELECAM", frm_telecam),
        ("DOWNLOAD_DIR/DOWNLOAD_DIR_BASE", download_dir or download_dir_base),
        ("STAGING_DOWNLOAD_DIR", staging_download_dir),
    ]:
        if not v:
            faltan.append(k)
    if input_source not in ("DIRECT", "GSHEET"):
        faltan.append("INPUT_SOURCE debe ser DIRECT o GSHEET")
    if input_source == "GSHEET":
        for k, v in [
            ...
            if not v:
                faltan.append(k)
    if input_source == "DIRECT" and not direct_centers:
        faltan.append("DIRECT_CENTERS o CENTERS_FILTER")
    if not tipo_atencion:
        faltan.append("TIPO_ATENCION")
    if not mostrar_por:
        faltan.append("MOSTRAR_POR")
    if not servicio:
        faltan.append("SERVICIO")
    if not actividad:
        faltan.append("ACTIVIDAD")
    if not subactividad:
        faltan.append("SUBACTIVIDAD")
    if not file_suffix:
    ...
    return {
        ...
        "USE_MONTH_SUBDIR": use_month_subdir,
        "STAGING_DOWNLOAD_DIR": staging_download_dir,
        "HEADLESS": headless,
...
# =========================
# Centros objetivo: entrada directa por defecto; Google Sheets queda como compatibilidad.
# =========================
MONTHS_ES = {
    1: "ENERO",
    2: "FEBRERO",
...
def month_output_dir_name(month_ctx: dict) -> str:
    try:
        month_num = int(str(month_ctx.get("LABEL", "")).split("-")[1])
    except Exception:
        dmy = month_ctx.get("MONTH_FIRST_DMY", "")
        month_num = int(dmy.split("/")[1])
    return f"{month_num:02d}. {MONTHS_ES.get(month_num, '')}"

def resolve_download_dir(config: dict, month_ctx: dict) -> str:
    configured = (config.get("DOWNLOAD_DIR") or "").strip()
    if configured:
        return configured
    base = (config.get("DOWNLOAD_DIR_BASE") or "").strip()
    if not base:
        raise RuntimeError("DOWNLOAD_DIR_BASE vacio")
    if config.get("USE_MONTH_SUBDIR", True):
        return str(Path(base) / month_output_dir_name(month_ctx))
    return base

def build_routes_for_month(config: dict, month_ctx: dict) -> List[Dict[str, str]]:
    routes = []
    if config.get("RUN_TELECAM"):
        final_dir = resolve_download_dir(config, month_ctx)
        ensure_dir(final_dir)
        routes.append({
            "code": "TELE_CAM",
            "name": "RPA Tele CAM",
            "url": config["FRM_TELECAM"],
...
def get_centros_from_direct(config: dict, logger=None) -> List[Dict[str, str]]:
    default_macro = config.get("DEFAULT_MACRO", "CENTRO")
    selected = []
    seen = set()
    for raw in config.get("DIRECT_CENTERS", []):
        centro = str(raw or "").strip()
        if not centro:
            continue
        if centro.isdigit():
            centro = centro.zfill(3)
            canon = str(int(centro))
        else:
            canon = centro
        if canon in seen:
            continue
        seen.add(canon)
        selected.append({"centro": centro, "macro": default_macro, "source": "DIRECT"})
    if logger:
        logger.info(f"DIRECT_INPUT | centros={len(selected)} | macro_default={default_macro}")
    return selected
...
def find_select_by_value(driver, candidates: List[Tuple[str, str]], expected_value: str, label: str):
    for by, selector in candidates:
        try:
            el = driver.find_element(by, selector)
            vals = [(o.get_attribute("value") or "").strip() for o in Select(el).options]
            if expected_value in vals:
                return el
        except Exception:
            pass
    for el in driver.find_elements(By.TAG_NAME, "select"):
        try:
            ...
            continue
    raise RuntimeError(f"No se encontró SELECT '{label}' con value={expected_value}.")

def wait_select_by_value(driver, candidates: List[Tuple[str, str]], expected_value: str, label: str, timeout: int = 25):
    wait = WebDriverWait(driver, timeout)
    return wait.until(lambda d: find_select_by_value(d, candidates, expected_value, label))
...
def find_select_formato_el(driver):
    for el in driver.find_elements(By.TAG_NAME, "select"):
        try:
            ...
            if {"pdf", "xls", "doc"}.issubset(vals):
                return el
        except Exception:
            continue
    raise RuntimeError("No se encontró SELECT 'Tipo Archivo' (pdf/xls/doc).")
...
def fill_form(driver, config: dict, month_ctx: dict, ruta: dict) -> None:
    ...
    tipo_atencion_val = config["TIPO_ATENCION"]
    mostrar_por_val = config["MOSTRAR_POR"]
    servicio_val = config["SERVICIO"]
    actividad_val = config["ACTIVIDAD"]
    subactividad_val = config["SUBACTIVIDAD"]
    formato_val = config["FORMATO_ARCHIVO"]

    wait.until(lambda d: len(d.find_elements(By.TAG_NAME, "select")) >= 5)

    el_tipo_atencion = wait_select_by_value(
        driver,
        [(By.ID, "tipoReporte"), (By.NAME, "tipoReporte")],
        tipo_atencion_val,
        "Tipo de Atencion",
    )
    Select(el_tipo_atencion).select_by_value(tipo_atencion_val)
    _trigger_change(driver, el_tipo_atencion)

    el_mostrar = wait_select_by_value(
        driver,
        [(By.NAME, "tipo"), (By.ID, "tipo")],
        mostrar_por_val,
        "Mostrar por",
    )
    Select(el_mostrar).select_by_value(mostrar_por_val)
    _trigger_change(driver, el_mostrar)

    el_serv = wait_select_by_value(
        driver,
        [(By.ID, "servicio"), (By.NAME, "servicio")],
        servicio_val,
        "Servicio",
    )
    Select(el_serv).select_by_value(servicio_val)
    _trigger_change(driver, el_serv)

    el_actividad = wait_select_by_value(
        driver,
        [(By.ID, "actividad"), (By.NAME, "actividad")],
        actividad_val,
        "Actividad",
    )
    Select(el_actividad).select_by_value(actividad_val)
    _trigger_change(driver, el_actividad)

    el_subactividad = wait_select_by_value(
        driver,
        [(By.ID, "subactividad"), (By.NAME, "subactividad")],
        subactividad_val,
        "SubActividad",
    )
    Select(el_subactividad).select_by_value(subactividad_val)
    _trigger_change(driver, el_subactividad)
...
def descargar_centro_tipo(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            final_path = publish_validated_download(logger, fullpath, final_dir, prefix, expected_suffix)
...
def main():
    ...
    rutas = build_routes_for_month(config, month_ctx)
    route_codes = [r["code"] for r in rutas]
    if not rutas:
        logger.info("RUN_SKIP | RUN_TELECAM=false. No hay nada que ejecutar.")
        sys.exit(1)

    if config["INPUT_SOURCE"] == "DIRECT":
        centros_meta = get_centros_from_direct(config, logger=logger)
    else:
    ...
    if not centros_meta:
        ...
        logger.info(f"INPUT | No hay IPRESS objetivo para el mes {mes_txt} | source={config['INPUT_SOURCE']}. Fin sin descargas.")
    ...
    send_run_email(
        logger,
        config,
        f"[{subject_status}] RPA Tele CAM | exit_code={exit_code}",
