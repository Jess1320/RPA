...
def build_job_config(code: str) -> JobConfig:
    ...
    defaults = {
        "PROG_PROF": {
            "name": "RPA Prog Prof",
            "workdir": "/home/cenate/rpa_prog_prof",
            "command": "scripts/run_rpa_prog_prof.sh",
            "logs_dir": "/home/cenate/rpa_prog_prof/logs",
            "run_name": "PROG_PROF_CRONICOS",
            "parser": "medico_no_medico",
        },
        "MENSUAL_CRONICOS": {
            "name": "RPA Mensual Cronicos",
            "workdir": "/home/cenate/rpa_mensual_cronicos",
            "command": "scripts/run_rpa_mensual_cronicos.sh",
            "logs_dir": "/home/cenate/rpa_mensual_cronicos/logs",
            "run_name": "CEXT_CRONICOS_MENSUAL",
            "parser": "mensual",
        },
    }[code]
...
def should_run_month_start_close(run_date: dt.date) -> bool:
    enabled = parse_bool(os.getenv("MONTH_START_CLOSE_ENABLED", "false"), default=False)
    if not enabled:
        return False
...
def run_job(execution: JobExecution, suppress_child_mail: bool) -> JobResult:
    ...
    if config.code == "MENSUAL_CRONICOS":
        env["DOWNLOAD_ONLY"] = "true"
        env["GSHEET_TABS"] = os.getenv("MENSUAL_CRONICOS_GSHEET_TABS", "CENACRON")
    if config.code == "PROG_PROF":
        env["GSHEET_TAB"] = os.getenv("PROG_PROF_GSHEET_TAB", "CENACRON")
...
def whatsapp_text(results: List[JobResult], run_date: dt.date) -> str:
    ...
    if not bad:
        ...
        return (
            f"Descargas de Cronicos {date_txt} listas: "
            f"{ok_txt} OK."
        )
    ...
    return f"Descargas de Cronicos {date_txt}: OK en {ok_txt}. Observaciones: {bad_txt}."
...
def build_email_body(
    ...
    lines = [
        "Reporte de ejecuciones Cronicos",
        "",
...
def write_report_files(results: List[JobResult], run_dir: str) -> List[str]:
    ...
    csv_path = os.path.join(run_dir, "reporte_cronicos.csv")
    json_path = os.path.join(run_dir, "reporte_cronicos.json")
...
def main() -> int:
    ...
    run_name = os.getenv("ORCH_RUN_NAME", "ORQUESTADOR_CRONICOS")
    ...
    order = split_list(os.getenv("JOB_ORDER", "PROG_PROF,MENSUAL_CRONICOS"))
    ...
    subject = f"[{subject_status}] Reporte RPAs Cronicos - {end_dt.strftime('%d/%m/%Y %H:%M')}"
    body = build_email_body(results, run_dir, start_dt, end_dt, notes=orch_notes)
    Path(run_dir, "correo_cronicos.txt").write_text(body, encoding="utf-8")
