...
def setup_run_logger(logs_dir: str, run_name: str, log_mode: str = "PROD"):
    ...
    logger = logging.getLogger("RPA_DX_MED_NMED")
...
def load_config() -> dict:
    ...
    download_dir_med = os.getenv("DOWNLOAD_DIR_MEDICO")
    download_dir_nmed = os.getenv("DOWNLOAD_DIR_NO_MEDICO")
    ...
    mostrar_por = os.getenv("MOSTRAR_POR", "4").strip()
    servicio = os.getenv("SERVICIO", "00").strip()
    formato_archivo = os.getenv("FORMATO_ARCHIVO", "xls").strip()

    # motor
    run_med = os.getenv("RUN_MEDICO", "true").lower() == "true"
    run_nmed = os.getenv("RUN_NO_MEDICO", "true").lower() == "true"
    ...
    # Paso 4: ejecución
    run_name = os.getenv("RUN_NAME", "MEDICO_NO_MEDICO")
    ...
    for k, v in [
        ...
        ("DOWNLOAD_DIR_MEDICO", download_dir_med),
        ("DOWNLOAD_DIR_NO_MEDICO", download_dir_nmed),
    ]:
        if not v:
            faltan.append(k)
    ...
    ensure_dir(logs_dir)
    ensure_dir(download_dir_med)
    ensure_dir(download_dir_nmed)
...
def find_select_mostrar_por_el(driver):
    try:
        return driver.find_element(By.CSS_SELECTOR, "select[name='tipo']")
    except Exception:
        pass
    for el in driver.find_elements(By.TAG_NAME, "select"):
        try:
            ...
            txts = [(o.text or "").strip().upper() for o in s.options]
            if "4" in vals and any("DIAGNOSTICO" in t for t in txts):
                return el
        except Exception:
            continue
    raise RuntimeError("No se encontró SELECT 'Mostrar por' (name='tipo').")

def find_select_tipo_atencion_el(driver):
    for el in driver.find_elements(By.TAG_NAME, "select"):
        try:
            s = Select(el)
            txts = [(o.text or "").strip().upper() for o in s.options]
            if any(t == "MEDICO" for t in txts) and any(t == "NO MEDICO" for t in txts):
                return el
        except Exception:
            continue
    raise RuntimeError("No se encontró SELECT 'Tipo de Atención' (MEDICO/NO MEDICO).")
    ...
    for el in driver.find_elements(By.TAG_NAME, "select"):
        try:
            ...
            for o in s.options:
                ...
                if val == servicio_val and "TODOS" in txt:
                    return el
        except Exception:
            continue
    raise RuntimeError("No se encontró SELECT 'Servicio' (00/TODOS).")
...
# =========================
def validate_filename(centro: str, month_ctx: dict, tipo: str, filename: str) -> tuple[bool, str]:
    if not filename or not filename.lower().endswith(".txt"):
        return False, "NOMBRE_INVALIDO"

    expected_suffix = "DxMedSer.txt" if tipo == "MEDICO" else "DxNoMedSer.txt"
...
# =========================
def open_form_for_center(logger, driver, config: dict, centro: str) -> None:
...
# =========================
def fill_form(driver, config: dict, month_ctx: dict, tipo: str) -> None:
    wait = WebDriverWait(driver, 25)

    mostrar_por_val = config["MOSTRAR_POR"]
    servicio_val = config["SERVICIO"]
    formato_val = config["FORMATO_ARCHIVO"]
    tipo_atencion_val = "1" if tipo == "MEDICO" else "2"

    wait.until(lambda d: len(d.find_elements(By.TAG_NAME, "select")) >= 2)

    el_mostrar = find_select_mostrar_por_el(driver)
    Select(el_mostrar).select_by_value(mostrar_por_val)
    _trigger_change(driver, el_mostrar)
    wait.until(lambda d: Select(el_mostrar).first_selected_option.get_attribute("value") == mostrar_por_val)

    el_tipo = find_select_tipo_atencion_el(driver)
    Select(el_tipo).select_by_value(tipo_atencion_val)
    _trigger_change(driver, el_tipo)

    el_serv = find_select_servicio_el(driver, servicio_val)
...
def descargar_centro_tipo(
    ...
    download_dir = config["DOWNLOAD_DIR_MEDICO"] if tipo == "MEDICO" else config["DOWNLOAD_DIR_NO_MEDICO"]
    expected_suffix = "DxMedSer.txt" if tipo == "MEDICO" else "DxNoMedSer.txt"
    prefix = f"{centro}_{month_ctx['MONTH_FIRST_YMD']}_{month_ctx['MONTH_LAST_YMD']}_"
    ...
    if config["CLEAN_BEFORE"]:
        clean_existing_finals(logger, download_dir, prefix, expected_suffix)
    remove_partial_with_prefix(logger, download_dir, prefix, unconfirmed_age_min=10)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            options, profile_dir = build_chrome_options(
                ...
                worker_id=f"{centro}-{tipo}-{user_label}-a{attempt}",
            )
            ...
            logger.info(f"RUN | centro={centro} | tipo={tipo} | user={user_label} | attempt={attempt}")

            open_form_for_center(logger, driver, login_config, centro)
            fill_form(driver, config, month_ctx, tipo)
            ...
            ok_name, reason = validate_filename(centro, month_ctx, tipo, archivo)
            if not ok_name:
                res["motivo"] = reason
                res["attempt"] = str(attempt)
                res["seconds"] = f"{time.time() - t0:.1f}"
...
def main():
    ...
    if not centros_meta:
        ...
        tipos_cfg = []
        if config["RUN_MEDICO"]:
            tipos_cfg.append("MEDICO")
        if config["RUN_NO_MEDICO"]:
            tipos_cfg.append("NO_MEDICO")
        send_run_email(
            logger,
            config,
            "[OK] RPA Medico y No Medico | exit_code=0",
            build_run_email_body(config, start_dt, end_dt, 0, month_ctx, tipos_cfg, 0, 0, [], [], []),
            logfile,
        )
        sys.exit(0)
    ...
    if not tipos:
        logger.info("❌ RUN_MEDICO y RUN_NO_MEDICO están en false. No hay nada que ejecutar.")
        sys.exit(1)
        ...
        for macro in VALID_MACROS:
            ...
            for centro in centros_macro:
                ...
                for tipo in tipos:
                    ...
                    r = descargar_centro_tipo(logger, config, centro, month_ctx, tipo, user_label, user, pwd)
                    resultados.append(r)
                    center_results[centro][tipo] = r["status"]

                    if r["status"] == "OK":
                        macro_ok += 1
                        logger.info(f"OK   | {centro} | {tipo} | {user_label} | {r['archivo']} | {r['seconds']}s")
                    else:
                        macro_fail += 1
                        logger.info(f"FAIL | {centro} | {tipo} | {user_label} | {r['motivo']} | {r['seconds']}s")

            logger.info(f"MACRO_END | {macro} | OK:{macro_ok} | FAIL:{macro_fail}")

    finally:
        purge_old_partials(logger, config["DOWNLOAD_DIR_MEDICO"], max_age_minutes=config["PURGE_PARTIALS_AGE_MIN"])
        purge_old_partials(logger, config["DOWNLOAD_DIR_NO_MEDICO"], max_age_minutes=config["PURGE_PARTIALS_AGE_MIN"])
        kill_chromedrivers()
        removed_profiles = cleanup_old_chrome_profiles(
            config["CHROME_PROFILE_BASE_DIR"],
            config["CHROME_PROFILE_MAX_AGE_HOURS"],
    ...
    # Resumen por centro (OK si TODOS los tipos OK)
    ...
    for centro in centros:
        ...
        if all(per.get(t) == "OK" for t in tipos):
    ...
    send_run_email(
        logger,
        config,
        f"[{subject_status}] RPA Medico y No Medico | exit_code={exit_code}",
